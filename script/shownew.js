...
async function main() {
    ...
    while(true) {
        ...
        list.forEach(element => {
            // console.log(element)
            newstakers.push(element)
            totamount += Number(element.amount)
            totreward += Number(element.reward)
        });

        queryMsg.start_after = list[list.length-1].address;
        console.log(newstakers.length)
    ...
    console.log("reward: " + totreward)
    fs.writeFileSync(`newstakerlist.json`, JSON.stringify(newstakers))
